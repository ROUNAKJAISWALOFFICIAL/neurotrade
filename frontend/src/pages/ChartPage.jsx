    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center gap-6 px-5 py-[8px] bg-[#111827] border-b border-white/[0.06] flex-shrink-0">
        {[
          ...
          ['Holdings', Object.keys(holdings).filter(s=>holdings[s].qty!==0).length + ' positions', 'text-white'],
...
function HoldingsTable({ holdings, prices }) {
  ...
  return (
    <table className="w-full text-[12px]">
      <thead>
        <tr>{['Symbol','Type','Qty','Avg','LTP','P&L'].map(h=><th key={h} className="text-left px-4 py-2 text-[10px] text-[#5a6478] uppercase tracking-[0.5px] font-medium">{h}</th>)}</tr>
      </thead>
      <tbody>
        {entries.map(([sym,h]) => {
          ...
          return (
            <tr key={sym} className="border-t border-white/[0.04] hover:bg-white/[0.02]">
              ...
              <td className={`px-4 py-[9px] text-[10px] font-medium px-2 py-1 rounded-full w-fit ${isShort ? 'bg-red-500/20 text-red-400' : 'bg-green-500/20 text-green-400'}`}>{isShort ? 'SHORT' : 'LONG'}</td>
              <td className="px-4 py-[9px] font-mono">{isShort ? -absQty : absQty}</td>
